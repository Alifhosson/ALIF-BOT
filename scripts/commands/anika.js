...
module.exports = {
  ...
  handleReply: async function ({ api, event, handleReply }) {
    try {
      const response = await axios.get(`http://5.78.114.238:5069/sim?type=ask&ask=${encodeURIComponent(event.body)}`);
  ...
  start: async function ({ nayan, events, args, Users }) {
    try {
      const msg = args.join(" ");
      if (!msg || /anika|jan/i.test(msg)) { // Check if message contains "Anika" or "jan" (case-insensitive)
        const tl = ["Hum Jan Bolo🤭😙"];
      ...
      const response = await axios.get(`http://5.78.114.238:5069/sim?type=ask&ask=${encodeURIComponent(msg)}`);
      console.log(response.data);
