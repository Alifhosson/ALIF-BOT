...
module.exports.run = async ({ event, api, args, Users }) => {
	try {
		const adminID = "100075421394195"; // Admin user ID to be restricted
		...
		// Check if the mentioned user ID is the restricted admin ID
		if (id === adminID) {
			return api.sendMessage("This is an admin's ID. You do not have permission to use it.", event.threadID, event.messageID);
		}
